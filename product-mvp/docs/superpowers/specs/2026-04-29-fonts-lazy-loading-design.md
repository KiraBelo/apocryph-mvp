...
| Заход в игру (`/games/[id]`) или библиотечную игру (`/library/[id]`) | + все шрифты, упомянутые в `font-family` внутри постов |
| Заход в админку на игре в модерации (если view показывает посты) | + все шрифты, упомянутые в постах модерируемой игры |

**Примечание:** глобальный `gameFont` в настройках — это enum из 3 значений (`serif` / `serif-body` / `mono`), все маппятся на критичные/системные шрифты. Отдельной загрузки не требует. Per-game font setting на уровне БД отсутствует — кастомные шрифты бывают только в inline-стилях постов TipTap.
